...
/**
 * CSS for --strip mode. Covers page chrome (header/nav), accessibility
 * toolbars, chat widgets, and floating CTAs/back-to-top buttons. Keep
 * additions brand-specific — broad class matches (e.g. [class*="chat"])
 * false-positive on legitimate content on many sites.
 */
const STRIP_CHROME_CSS = `
  /* Accessibility widgets */
  access-widget-ui,
  [id*="userway" i], [class*="userway" i],
  [id*="acsb-" i], [class*="acsb-" i], .acsb-trigger, .acsb-widget,
  #INDMenu, .INDmenu-btn, [class*="equalweb" i],
  [class*="usablenet" i],
  [class*="accessibility-widget" i], [id*="accessibility-widget" i],
  [aria-label*="accessibility menu" i],
  [aria-label*="accessibility options" i],

  /* Chat widgets */
  #intercom-frame, .intercom-launcher, .intercom-launcher-frame,
  [class*="intercom-lightweight" i], [class*="intercom-launcher" i],
  #drift-widget, [class*="drift-frame" i],
  #crisp-chatbox, [class*="crisp-client" i],
  #tawkchat-container, [class*="tawk-min-container" i], [class*="tawkchat" i],
  #hubspot-messages-iframe-container, [class*="hubspot-messages" i],
  #tidio-chat, [class*="tidio-chat" i],
  #launcher[title*="chat" i], [class*="zEWidget-launcher" i], [id*="zendesk-chat" i],
  #chat-widget-container, [class*="livechatWidget" i],
  .fb_dialog, [class*="fb-customerchat" i],

  /* Back-to-top / floating CTAs */
  [class*="back-to-top" i], [id*="back-to-top" i],
  [class*="scroll-top" i], [class*="scrolltop" i],
  [aria-label*="back to top" i], [aria-label*="scroll to top" i] {
    display: none !important;
    visibility: hidden !important;
    opacity: 0 !important;
    pointer-events: none !important;
...
/**
 * --strip mode: hide page chrome, a11y toolbars, chat widgets, floating CTAs.
 * Runs as a second pass after cleanPage — strip is aggressive; clean is not.
 * Also sweeps fixed/sticky elements larger than the cleanPage threshold to
 * catch unbranded floating widgets that don't match any brand selector.
 */
async function stripChrome(page) {
  await page.addStyleTag({ content: STRIP_CHROME_CSS });

  await page.evaluate(() => {
    // Remove custom-element widgets that enforce `display: block` via their
    // own shadow-DOM stylesheets and ignore outer CSS. These come back cleanly
    // when the element itself is gone — observed on AccessiBe, UserWay, etc.
    document.querySelectorAll("access-widget-ui, userway-widget").forEach((el) => el.remove());

    // Hide page-chrome <header> and <nav> elements. We can't use
    // `body > header` because many themes nest them in wrapper divs
    // (WordPress/Fusion, Astra, Divi, etc.), so instead key on the
    // element's absolute document position: anything whose top is in
    // the first 200px of the document is page-level chrome. Fixed/
    // sticky ones always qualify regardless of position.
    document.querySelectorAll("header, nav, [role='banner'], [role='navigation']").forEach((el) => {
      const rect = el.getBoundingClientRect();
      const absTop = rect.top + window.scrollY;
      const pos = window.getComputedStyle(el).position;
      if (absTop < 200 || pos === "fixed" || pos === "sticky") {
        el.style.setProperty("display", "none", "important");
      }
    });

    // Hide any fixed/sticky element that isn't a modal-sized overlay.
    // Skip elements that cover most of the viewport (likely a real modal / content).
    document.querySelectorAll("*").forEach((el) => {
      const s = window.getComputedStyle(el);
...
async function preparePage(page, { clean, strip, fullPage, delay, maxWidth, reveal }) {
  ...
  // Strip runs LAST so chrome that lazy-loads on scroll (headers, sticky
  // widgets, chat bubbles) is hit after it has appeared. Running it earlier
  // lets site JS re-render the widgets before the screenshot.
  if (strip) await stripChrome(page);
}
...
function normalizeOptions(opts = {}) {
  return {
    ...
    strip: opts.strip === true || opts.strip === "true",
...
async function capturePdf(opts, onProgress = noop) {
  ...
  try {
    ...
    await preparePage(page, { clean: n.clean, strip: n.strip, fullPage: true, delay: n.delay, maxWidth: 0, reveal: false });
